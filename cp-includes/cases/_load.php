<?php

//Загрузчик компонентов модуля Кейсы

include_once 'includes/add_cases.php';
include_once 'includes/tax_results.php';
include_once 'includes/tax_functions.php';
include_once('includes/cases_api.php');
include_once('includes/cases_view.php');
include_once('includes/cases_shortcode_manage.php');
include_once('includes/cases_view_admin.php');
include_once 'includes/filter_open_closed.php';
include_once 'includes/add_nav_items.php';
include_once 'includes/add_panel_manage_case.php';
include_once 'includes/dossier.php';
include_once 'includes/cases_and_deadlines.php';


include_once 'includes/comments_and_notice/add_case_members_to_notify.php';
include_once 'includes/comments_and_notice/CP_DeadLineComment.php';
include_once 'includes/comments_and_notice/CP_NoticeComment.php';

include_once 'includes/visits/_load.php';
